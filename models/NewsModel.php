...
class NewsModel
{
	...
	{
		$bd = DB::connect();
		$query = mysql_query("SELECT news_comment.id as 'id', id_new, comment, news.title as 'title', news_comment.datetime, publish, users.name, users.surname
		 FROM news_comment LEFT JOIN users ON news_comment.id_user=users.id 
		 LEFT JOIN news ON news.id = news_comment.id_new
		 WHERE news_comment.publish = 0");
		$comments = array();
		$i = 0;
	...
	public function get_edit_rights()
	{
		...
		$query = mysql_query("SELECT * FROM users WHERE id='$user_id' and type='2'");
		if ($query)
	...
	public function get_comments($id)
	{
		...
		$query_comments = mysql_query("SELECT * FROM news_comment 
			LEFT JOIN users ON news_comment.id_user = users.id WHERE id_new = '$id' and publish=1 ORDER BY datetime DESC");
		$i=0;
